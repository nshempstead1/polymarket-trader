...
def get_wallet_balance():
    """Get wallet USDC.e balance and position value."""
    WALLET = os.getenv('WALLET_ADDRESS', '0x769Bb0B16c551aA103F8aC7642677DDCc9dd8447')
    try:
        resp = requests.get(
        ...
        usdc_balance = contract.functions.balanceOf(Web3.to_checksum_address(WALLET)).call() / 1e6
        
        return {'usdc': usdc_balance, 'positions': position_value, 'total': usdc_balance + position_value, 'count': position_count}
    except Exception as e:
        return {'usdc': 0, 'positions': 0, 'total': 0, 'count': 0, 'error': str(e)}
...
def show_overview(journal: TradeJournal):
    ...
    print(f"\n{C.BOLD}  💰 Wallet Balance{C.RESET}")
    hr()
    if bal.get('error'):
        print(f"  {C.DIM}Error fetching balance: {bal['error']}{C.RESET}")
    else:
        print(f"  USDC.e:          {C.G}${bal['usdc']:.2f}{C.RESET}")
        print(f"  Positions ({bal['count']:>2d}):  {C.C}${bal['positions']:.2f}{C.RESET}")
        print(f"  {C.BOLD}Total Value:     ${bal['total']:.2f}{C.RESET}")
